...
def run_scraper():
  ...
  # Explicitly isolate the table containing Opta prediction header tags
  predicted_table = None
  for tbl in tables:
    header_text = tbl.get_text().lower()
    if any(
        k in header_text
        for k in ["xpts", "title", "promotion", "rel", "%", "playoffs"]
    ):
      predicted_table = tbl
      break

  if not predicted_table:
    # If explicit keywords fail, pick the table that has percentage symbols or the second table
