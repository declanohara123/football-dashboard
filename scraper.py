...
def run_scraper():
  ...
  # Explicitly find table containing Opta prediction model columns
  ...
  # If explicit keyword match fails, filter out standard league table (which lacks xPts)
  if not predicted_table:
    valid_tables = [
        t for t in tables if "xpts" in t.get_text().lower() or len(t.find_all("th")) > 6
    ]
    if valid_tables:
      predicted_table = valid_tables[0]
    else:
      # Use second table if multiple exist, as live table is usually first
  ...
  if not existing_df.empty and "date" in existing_df.columns:
    # Remove existing entries for this date so it overwrites correctly
  ...
  print(
      f"Successfully scraped and updated {len(new_df)} predicted team entries"
      f" for date: '{date_str}'."
  )
